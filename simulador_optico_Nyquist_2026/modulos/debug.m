...
%% 3. ECUALIZADOR (Convergencia, SNR, Evolución y Respuesta Temporal)
    figure('Name', '3. Dinámica del Ecualizador FFE', 'Color', 'w', 'Position', [200, 200, 1100, 700]);
    ...
    % --- Subplot C: Evolución de los Taps principales (Líneas) ---
    subplot(2,2,3);
    downsample_factor = 20; 
    taps_to_plot = abs(rx.htaps_log(:, 1:downsample_factor:end));
    iter_axis = (1:size(taps_to_plot, 2)) * FRAME_LOG * downsample_factor;
    
    energia_final_taps = taps_to_plot(:, end);
    ...
    for i = 1:num_taps_to_show
        ...
        plot(iter_axis, taps_to_plot(idx_tap, :), 'LineWidth', 1.5, ...
             'Color', colores(i,:), ...
             'DisplayName', sprintf('Tap %d', idx_tap));
    end
    
    taps_secundarios = setdiff(1:cfg.NTAPS_ffe, taps_principales);
    ruido_promedio_taps = mean(taps_to_plot(taps_secundarios, :), 1);
    plot(iter_axis, ruido_promedio_taps, 'k--', 'LineWidth', 1, ...
         'DisplayName', 'Ruido de Taps (Promedio)');
    
    grid on; box on;
    xline(cfg.t4_ffe_dd, 'r--', 'DD');
    xlabel('Iteraciones', 'FontWeight', 'bold');
    ylabel('Magnitud Absoluta', 'FontWeight', 'bold');
    title('Evolución (Top 5 Taps)');
    legend('Location', 'best');
    hold off;

    % --- Subplot D: Respuesta al Impulso del FFE (Forma Temporal) ---
    subplot(2,2,4);
    
    % Usamos los taps finales reales (complejos) y graficamos su magnitud
    ...
    % Remarcar el tap central original (usualmente el impulso inicial)
    ...
    title('Forma Temporal del Ecualizador (Taps Finales)');
    
    % Añadir un texto flotante con información clave
...
%% 4. RECUPERACIÓN DE PORTADORA (PLL Y CONSTELACIONES)
    if cfg.en_carrier_recovery
        ...
        slicer_in_final = rx.slicer_in_log(end-N_tail:end);
        phase_acc_final = rx.phase_acc_log(end-N_tail:end);
