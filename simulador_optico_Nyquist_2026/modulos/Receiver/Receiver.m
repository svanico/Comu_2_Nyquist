function [o_data_rx] = Receiver(i_rx, i_cfg_s, ak)
    ...
    error_base_log = zeros(ceil(Lsymbs/FRAME_LOG_1x),1);
    ...
    for idx=1:length(agc_out) %a tasa de sobremuestreo
        ...
        if mod(idx,ovs_ffe)==0 % calculo el resto, si da 0, continuo (downsampling)
            ...
            if idx_new < t4_ffe_dd
                ...
                error = slicer_in_raw*(abs(slicer_in_raw).^2 - R_CMA);
                step = cma_step;
            else
                % Etapa 5 usa LMS (DD)
            ...
            if mod(idx_new,FRAME_LOG_1x)==0
                ...
                error_base_log(idx_log) = error_base;
    ...
    o_data_rx.error_base_log = error_base_log;
    ...
    o_data_rx.htaps_log = htaps_log;
    o_data_rx.agc_in = dsp_in;   % Para comparar la entrada vs salida del AGC
    o_data_rx.agc_out = agc_out;
end