...
kp1 = load('penalidad_Kp_1e-04.mat');
kp2 = load('penalidad_Kp_3e-04.mat');
kp3 = load('penalidad_Kp_1e-03.mat');
kp4 = load('penalidad_Kp_3e-03.mat');
...
semilogx(kp2.f_jitter_vec, kp2.penalidad_SNR_dB, '-o', ...
    'LineWidth', 1.5, 'DisplayName', 'K_p = 3e-4');
...
semilogx(kp4.f_jitter_vec, kp4.penalidad_SNR_dB, '-o', ...
    'LineWidth', 1.5, 'DisplayName', 'K_p = 3e-3');

grid on;
xlabel('Frecuencia de Jitter [Hz]');
ylabel('Penalidad de SNR [dB]');
title('Penalidad de SNR vs Frecuencia de Jitter - Detector 4th Power');
